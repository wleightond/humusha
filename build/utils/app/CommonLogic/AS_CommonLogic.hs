...
{- |
Module      :  ./CommonLogic/AS_CommonLogic.hs
...
instance GetRange SENTENCE where
    getRange (Quant_sent _ [_] _ idrange) = idrange
    getRange (Bool_sent _ idrange) = idrange
    getRange (Atom_sent _ idrange) = idrange
    getRange (Comment_sent _ _ idrange) = idrange
    getRange (Irregular_sent _ idrange) = idrange
...
instance GetRange ATOM where
    getRange (Equation term1 term2)     = appRange (getRange term1) (getRange term2)
    getRange (Atom     term [term_seq]) = appRange (getRange term)  (getRange term_seq)
...
instance GetRange TERM where
    getRange (Funct_term   _ [_] idrange) = idrange
    getRange (Comment_term _  _  idrange) = idrange
    getRange (That_term    _     idrange) = idrange
...
instance GetRange TERM_SEQ where
    getRange (Term_seq term) = getRange term
