...
{- |
Module      :  ./Common/AS_Annotation.hs
...
instance GetRange Annotation where
    getRange (Unparsed_anno annote_word annote_text range) = range
    getRange (Display_anno id [(display_format, string)] range) = range
    getRange (List_anno id1 id2 id3 range) = range
    getRange (Number_anno id range) = range
    getRange (Float_anno id1 id2 range) = range
    getRange (String_anno id1 id2 range) = range
    getRange (Prec_anno precRel [id1] [id2] range) = range
    getRange (Assoc_anno assocEither [id] range) = range
    getRange (Label [string] range) = range
    getRange (Prefix_anno [(string, iri)] range) = range
    getRange (Semantic_anno semantic_anno range) = range
